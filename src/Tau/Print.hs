...
prettyOp :: Int -> OpF (Fix ExprF) -> Doc a
prettyOp n = \case
...
data Op = AddOp | MulOp deriving (Eq, Show)

flattenOp :: Op -> [Fix ExprF] -> [Expr]
flattenOp _ [] = []
flattenOp op (x:xs) = 
    case unfix x of
...
instance Pretty Class where
    pretty (Class name ty) = pretty name <+> pretty ty

instance Pretty Scheme where
    pretty (Forall vars clcs ty) = forall <> classes <> pretty ty
      where
        forall 
          | null vars = mempty
...
prettyArg data_@Data{}   = parens (pretty data_)
