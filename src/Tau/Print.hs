...
instance Pretty TyCl where
    pretty (TyCl name ty) = pretty name <+> pretty ty

instance (Pretty a) => Pretty (TyCl2 a) where
    pretty (TyCl2 name ty) = pretty name <+> pretty ty

instance Pretty ClType where
    pretty (ClType clcs ty) = classes <> pretty ty where
        classes
          | null clcs = mempty
          | otherwise = tupled (pretty <$> clcs) <+> "=>" <> space

instance (Pretty a) => Pretty (ClType2 a) where
    pretty (ClType2 clcs ty) = classes <> pretty ty where
        classes
          | null clcs = mempty
          | otherwise = tupled (pretty <$> clcs) <+> "=>" <> space

instance Pretty Scheme where
    pretty (Forall vars clcs ty) = forall <> pretty (ClType clcs ty) where
        forall
          | null vars = mempty
