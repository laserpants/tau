...
prettyAst = cata $ \case
    ...
    EPat t [] eqs            -> Node "fun" []
    EPat t exs eqs           -> Node "match" (exs <> [Node "with" (prettyClauseTree =<< eqs)]) 
...
prettyClauseTree :: Clause (Pattern t) (Tree (Doc a)) -> [Tree (Doc a)]
prettyClauseTree (Clause ps exs e) = 
    (prettyPatternTree <$> ps) <> whens <> [Node "=>" [e]]
  where
    whens 
        | null exs  = []
