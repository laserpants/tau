...
runTest4 = 
    case runInfer2 as (infer22 mempty test4) of
        Right (tree, sub) -> mapTags (apply sub) tree
        Left e -> error e
  where
...
runTest5 = 
    case runInfer2 as (infer22 mempty test5) of
        Right (tree, sub) -> mapTags (apply sub) tree
        Left e -> error e
  where
    as = [As2 "id" (sForall kStar "a" [] (sScheme (tGen 0 `tArr` tGen 0))), As2 "(,)" (sForall kStar "a" [] (sForall kStar "b" [] (sScheme (tGen 1 `tArr` tGen 0 `tArr` tApp (tApp (tCon (kArr kStar (kArr kStar kStar)) "(,)") (tGen 1)) (tGen 0)))))]
...
runTest6 = 
    case runInfer2 as (infer22 mempty test6) of
        Right (tree, sub) -> mapTags (apply sub) tree
        Left e -> error e
  where
...
runTest7 = 
    case runInfer2 as (infer22 mempty test7) of
        Right (tree, sub) -> mapTags (apply sub) tree
        Left e -> error e
  where
    as = [As2 "id" (sForall kStar "a" [] (sScheme (tGen 0 `tArr` tGen 0))), As2 "(,)" (sForall kStar "a" [] (sForall kStar "b" [] (sScheme (tGen 1 `tArr` tGen 0 `tArr` tApp (tApp (tCon (kArr kStar (kArr kStar kStar)) "(,)") (tGen 1)) (tGen 0)))))]
...
runTest8 = 
    case runInfer2 as (infer22 mempty test8) of
        Right (tree, sub) -> mapTags (apply sub) tree
        Left e -> error e
  where
    as = [As2 "id" (sForall kStar "a" [] (sScheme (tGen 0 `tArr` tGen 0))), As2 "(,)" (sForall kStar "a" [] (sForall kStar "b" [] (sScheme (tGen 1 `tArr` tGen 0 `tArr` tApp (tApp (tCon (kArr kStar (kArr kStar kStar)) "(,)") (tGen 1)) (tGen 0)))))]
...
runTest9 = 
    case runInfer2 as (infer22 mempty test9) of
        Right (tree, sub) -> mapTags (apply sub) tree
        Left e -> error e
  where
    as = [As2 "id" (sForall kStar "a" [] (sScheme (tGen 0 `tArr` tGen 0))), As2 "(,)" (sForall kStar "a" [] (sForall kStar "b" [] (sScheme (tGen 1 `tArr` tGen 0 `tArr` tApp (tApp (tCon (kArr kStar (kArr kStar kStar)) "(,)") (tGen 1)) (tGen 0)))))]
...
runTest10 = 
    case runInfer2 as (infer22 mempty test10) of
        Right (tree, sub) -> mapTags (apply sub) tree
        Left e -> error e
  where
    as = [As2 "id" (sForall kStar "a" [] (sScheme (tGen 0 `tArr` tGen 0))), As2 "(,)" (sForall kStar "a" [] (sForall kStar "b" [] (sScheme (tGen 1 `tArr` tGen 0 `tArr` tApp (tApp (tCon (kArr kStar (kArr kStar kStar)) "(,)") (tGen 1)) (tGen 0)))))]
...
test11 = 
    letExpr () (recPat () [Field () "name" (varPat () "x"), Field () "id" (varPat () "id")]) 
               ...
               (varExpr () "x")
runTest11 = 
    case runInfer2 as (infer22 mempty test11) of
        Right (tree, sub) -> mapTags (apply sub) tree
        Left e -> error e
  where
    as = [] 
...
test12 = 
    letExpr () (conPat () "(,)" [varPat () "x", varPat () "y"]) 
               ...
               (varExpr () "x")
runTest12 = 
    case runInfer2 as (infer22 mempty test12) of
        Right (tree, sub) -> mapTags (apply sub) tree
        Left e -> error e
  where
    as = [As2 "(,)" (sForall kStar "a" [] (sForall kStar "b" [] (sScheme (tGen 1 `tArr` tGen 0 `tArr` tApp (tApp (tCon (kArr kStar (kArr kStar kStar)) "(,)") (tGen 1)) (tGen 0)))))]
...
runTest13 = 
    case runInfer2 as (infer22 mempty test13) of
        Right (tree, sub) -> mapTags (apply sub) tree
        Left e -> error e
  where
...
runTest14 = 
    case runInfer2 as (infer22 mempty test14) of
        Right (tree, sub) -> mapTags (apply sub) tree
        Left e -> error e
  where
    as = [As2 "Some" (sForall kStar "a" [] (sScheme (tGen 0 `tArr` tApp (tCon (kArr kStar kStar) "Option") (tGen 0)))), As2 "(,)" (sForall kStar "a" [] (sForall kStar "b" [] (sScheme (tGen 1 `tArr` tGen 0 `tArr` tApp (tApp (tCon (kArr kStar (kArr kStar kStar)) "(,)") (tGen 1)) (tGen 0)))))]
...
runTest17 = 
    case runInfer2 as (infer22 mempty test17) of
        Right (tree, sub) -> mapTags (apply sub) tree
        Left e -> error e
  where
    as = [As2 "Some" (sForall kStar "a" [] (sScheme (tGen 0 `tArr` tApp (tCon (kArr kStar kStar) "Option") (tGen 0)))), As2 "(,)" (sForall kStar "a" [] (sForall kStar "b" [] (sScheme (tGen 1 `tArr` tGen 0 `tArr` tApp (tApp (tCon (kArr kStar (kArr kStar kStar)) "(,)") (tGen 1)) (tGen 0)))))]
...
runTest18 = 
    case runInfer2 as (infer22 mempty test18) of
        Right (tree, sub) -> mapTags (apply sub) tree
        Left e -> error e
  where
...
instantiate2 scheme = do
    traceShowM scheme
    ts <- reverse . zipWith tVar kinds <$> supplies (length kinds)
    pure (replaceBound ts ty, predicates ts)
  where
    ...
    predicates ts = snd (cata alg scheme)
      where
        alg :: Algebra SchemeF (Int, [Predicate])
        alg = \case
            Scheme t              -> (0, [])
            Forall _ _ ps (n, qs) -> (succ n, (toPred n <$> ps) <> qs)

        toPred n p = InClass p (ts !! n)
...
--typeOf4 :: Expr (Type, Int) p q -> (Type, Int)
--typeOf4 = getTag

typeOf5 :: Expr (Type, [Predicate]) p q -> Type
typeOf5 e = fst (getTag e :: (Type, [Predicate]))

typeOf6 :: Pattern (Type, [Predicate]) -> Type
typeOf6 e = fst (getTag e :: (Type, [Predicate]))
...
infer22
  ...
  => ClassEnv a 
  -> PatternExpr t 
  -> StateT Substitution m (PatternExpr (Type, [Predicate]))
infer22 env = cata alg 
  where 
    alg expr = do
        newTy <- newTVar kStar
        case expr of
            EVar _ var -> do
                ...
                unify ty newTy
                pure (varExpr (newTy, ps) var)

            ECon _ con exprs -> do
                as <- ask
                (ty, ps) <- lookupScheme con as >>= instantiate2
                es <- sequence exprs
                unify ty (foldr tArr newTy (typeOf5 <$> es))
                pure (conExpr (newTy, ps) con es)
            ...
            EIf _ cond tr fl -> do
                ...
                e2 <- tr
                e3 <- fl
                unify (typeOf5 e1) tBool
                unify (typeOf5 e2) (typeOf5 e3)
                unify newTy (typeOf5 e2)
                pure (ifExpr (newTy, []) e1 e2 e3)

            EOp  _ (OAnd a b) -> inferLogicOp22 OAnd a b
            EOp  _ (OOr  a b) -> inferLogicOp22 OOr a b
            EOp  _ _ -> undefined

            EMat _ exs eqs -> do
                es1 <- sequence exs
                es2 <- sequence (inferClause22 newTy es1 <$> eqs)
                pure (matExpr (newTy, []) es1 es2)

            ERec _ fields -> do
                let (_, ns, fs) = unzip3 (sortedFields fields)
                es <- sequence fs
                let tfs = zipWith (\n e -> Field (getTag e) n e) ns es
                unify newTy (foldl tApp (recordConstructor ns) (typeOf5 <$> es))
    ...
    alg expr = do
        ...
        case expr of
            ...
            ERec _ fields -> do
                ...
                unify newTy (foldl tApp (recordConstructor ns) (typeOf2 <$> es))
                pure (recExpr newTy tfs)

inferClause22
  :: (MonadSupply Name m, MonadReader [Assumption2] m, MonadError String m) 
  => Type
  -> [PatternExpr (Type, [Predicate])]
  -> Clause (Pattern t) (StateT Substitution m (PatternExpr (Type, [Predicate]))) 
  -> StateT Substitution m (Clause (Pattern (Type, [Predicate])) (PatternExpr (Type, [Predicate])))
inferClause22 ty es1 clause@(Clause ps _ _) = do
    (tps, as) <- sequenced (inferPattern22 <$> ps)
    let (Clause _ exs e) = local (foldr (:) as) <$> clause
    forM_ exs (>>= unify tBool . typeOf5)
    forM_ (zip tps es1) (\(p, e2) -> unify (typeOf6 p) (typeOf5 e2)) 
    es <- sequence exs
    e1 <- e
    unify ty (typeOf5 e1)
...
inferPattern22 
  :: (MonadSupply Name m, MonadReader [Assumption2] m, MonadError String m) 
  => Pattern t 
  -> StateT Substitution m (Pattern (Type, [Predicate]), [Assumption2])
inferPattern22 = cata alg
  where
    alg pat = do
        newTy <- newTVar kStar
        case pat of
            ...
            PCon _ con ps -> do
                ...
                let as = as1 <> as2
                (ty, ps) <- lookupScheme con as >>= instantiate2
                unify ty (foldr tArr newTy (typeOf6 <$> trs))
            ...
            PRec _ fields -> do
                ...
                unify newTy (foldl tApp (recordConstructor ns) (fst <$> ((getTag <$> ps) :: [(Type, [Predicate])])))
    ...
    alg pat = do
        ...
        case pat of
            ...
            PRec _ fields -> do
                ...
                (ps, as) <- second concat . unzip <$> sequence fs
                let tfs = zipWith (\n p -> Field (getTag p) n p) ns ps
                unify newTy (foldl tApp (recordConstructor ns) (getTag <$> ps))
                pure (recPat newTy tfs, as)
